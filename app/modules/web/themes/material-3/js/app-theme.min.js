...
sysPass.Theme = function (log) {
    ...
    const passwordDetect = function ($container) {
        ...
        $container.find(".passwordfield__input").each(function () {
            ...
            $thisParent.next(".password-actions")
                .prepend(`<i id='password-level-${targetId}' class="showpass material-icons password-level" data-input-target='${targetId}' data-level-msg= '' title="${sysPassApp.config.LANG[32]}">remove_red_eye</i>`)
                .prepend(btnMenu);
            ...
            if ($passwordRepeat.length > 0) {
                const $matchError = $('<span class="pass-match-error" style="display:none;color:#ef5350;font-size:12px;margin-top:4px;">Passwords do not match</span>');
                $passwordRepeat.parent().after($matchError);
                const validateMatch = function () {
                    const v1 = $this.val();
                    const v2 = $passwordRepeat.val();
                    const mismatch = v2.length > 0 && v1 !== v2;
                    $matchError.toggle(mismatch);
                    $passwordRepeat.parent().toggleClass("is-invalid", mismatch);
                };
                $this.on("blur input", validateMatch);
                $passwordRepeat.on("blur input", validateMatch);
            }
            ...
            // Crear evento para mostrar clave generada/introducida (hold to reveal)
            $passwordActions
                .find(".showpass")
        ...
        $container.find(".passwordfield__input-show").each(function () {
            ...
            // Crear evento para mostrar clave (hold to reveal)
    ...
    const viewsTriggers = {
        ...
        search: function () {
            ...
            $frmSearch.find("button.btn-clear").on("click", function (e) {
                const $favBtn = $(".icon-searchfav");
                $favBtn.removeClass("is-active");
                $favBtn.find("i").removeClass("mdl-color-text--amber-A200").text("star_border");
            });
            ...
            $resContent.on("click", "#data-search-header .header-column.sortable", function () {
                const $this = $(this);
                ...
                // Tags bar is always visible, just add the tag to the filter
                $tagsSelect.selectize.addItem($(this).data("tag-id"), false);
            });

            // Clear tags button
            $frmSearch.find("#btn-clear-tags").on("click", function () {
                $tagsSelect.selectize.clear();
                $frmSearch.submit();
            });
        },
    ...
    const html = {
        ...
        tabs: {
            add: function (header, index, title, isActive) {
                ...
                $header.find("a[href='#tabs-" + index + "']").on("click", function(e) {
                    e.preventDefault();
                    const $this = $(this);
                    const targetId = $this.attr("href");
                    const $tabsContainer = $this.closest(".mdl-tabs");
                    ...
                    $tabsContainer.find(targetId).addClass("is-active");
                });
            }
        }
    };
     ...
     */
    const init = function () {
        // Enable row click selection for data tables
        $(document).on("click", ".mdl-data-table--selectable tbody tr", function (e) {
            // Don't toggle selection if clicking on interactive elements
            const $target = $(e.target);
            if ($target.closest("button, a, input, label.mdl-checkbox, .cell-actions, .mdl-menu").length > 0) {
                return;
            }

            // Find the checkbox and toggle it
            const $row = $(this);
            const checkbox = $row.find("td:first-child .mdl-checkbox__input")[0];

            if (checkbox) {
                // Toggle checkbox and dispatch change event for MDL
