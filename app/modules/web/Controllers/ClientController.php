...
final class ClientController extends ControllerBase implements CrudControllerInterface
{
     ...
     */
    protected function setViewData($clientId = null): void
    {
    ...
    public function deleteAction($id = null)
    {
        try {
            ...
            // Check if this is a foreign key constraint error (1451)
            if (str_contains($e->getHint() ?? '', '1451') || str_contains($e->getHint() ?? '', 'AccountHistory')) {
                $ids = $id !== null ? [$id] : $this->getItemsIdFromRequest($this->request);
                return $this->returnJsonResponseData(
                    [
                        'action' => 'client/deleteWithHistory',
                        'actionDescription' => __u('Clear history and delete'),
                        'ids' => $ids,
                    ],
                    JsonResponse::JSON_ERROR,
                    __u('Cannot delete: this item has account history records'),
                    [__u('Would you like to clear the account history for these clients and then delete them?')]
                );
            }
    ...
    /**
     * Delete action with history clearing
     *
     * @param $id
     *
     * @return bool
     */
    public function deleteWithHistoryAction($id = null)
    {
        try {
            $this->checkSecurityToken($this->previousSk, $this->request);
            ...
            $accountHistoryService = $this->dic->get(\SP\Services\Account\AccountHistoryService::class);

            if ($id === null) {
                $ids = $this->getItemsIdFromRequest($this->request);

                // First clear the account history for these clients
                $deletedHistory = $accountHistoryService->deleteByClientIdBatch($ids);
                ...
                $this->eventDispatcher->notifyEvent(
                    ...
                    new Event($this, EventMessage::factory()
                        ->addDescription(__u('Clients deleted'))
                        ->addDetail(__u('History records cleared'), $deletedHistory))
                );

                return $this->returnJsonResponse(
                    JsonResponse::JSON_SUCCESS,
                    __u('Clients deleted'),
                    [sprintf(__('%d history records were cleared'), $deletedHistory)]
                );
            }

            // First clear the account history for this client
            $deletedHistory = $accountHistoryService->deleteByClientIdBatch([$id]);
