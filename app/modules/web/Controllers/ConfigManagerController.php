...
final class ConfigManagerController extends ControllerBase
{
    ...
    protected function getTabs(): void
    {
        ...
        if ($this->checkAccess(Acl::CONFIG_MAIL)) {
            $this->tabsHelper->addTab($this->getFeedbackConfig());
        }
     ...
     */
    protected function getConfigGeneral(): \SP\Mvc\View\Components\DataTab
    {
     ...
     * @throws SPException
     */
    protected function getAccountConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
        ...
        $mimeTypesAvailable = array_map(fn(array $value) => $value['type'], $this->dic->get(MimeTypes::class)->getMimeTypes());
     ...
     * @throws CheckException
     */
    protected function getWikiConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
     ...
     * @throws CheckException
     */
    protected function getLdapConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
     ...
     * @throws NotFoundExceptionInterface
     */
    protected function getMailConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
    ...
    protected function getFeedbackConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
        $template->addTemplate('feedback');

        return new DataTab(__('Feedback'), $template);
    }
     ...
     */
    protected function getEncryptionConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
     ...
     * @throws CheckException
     */
    protected function getBackupConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
     ...
     * @throws NotFoundExceptionInterface
     */
    protected function getImportConfig(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
        ...
        $allUsers = UserService::getItemsBasic();
        $defaultUserId = $this->userData->getId();

        foreach ($allUsers as $user) {
            if (method_exists($user, 'isAdminApp') && $user->isAdminApp()) {
        ...
        $template->assign(
            'users',
            SelectItemAdapter::factory($allUsers)
                ->getItemsFromModelSelected([$defaultUserId])
        );
     ...
     * @throws ServiceException
     */
    protected function getInfo(): \SP\Mvc\View\Components\DataTab
    {
        $template = clone $this->view;
        $template->setBase('config');
