...
final class UserSettingsGeneralController extends SimpleControllerBase
{
    ...
    public function saveAction()
    {
        try {
            ...
            $userPreferencesData->setStickySearchBar($this->request->analyzeBool('sticky_search_bar', false));
            ...
            // Update session locale so language change takes effect immediately
            $newLang = $userPreferencesData->getLang();
            if (!empty($newLang)) {
                $this->session->setLocale($newLang);
            }
