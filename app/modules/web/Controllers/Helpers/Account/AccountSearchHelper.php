...
final class AccountSearchHelper extends HelperBase
{
     ...
     * @param bool $isAjax
     */
    public function setIsAjax($isAjax): void
    {
        $this->isAjax = $isAjax;
    }
    ...
    public function getSearchBox(): void
    {
        ...
        $this->view->assign(
            ...
            SelectItemAdapter::factory(
                $this->dic->get(ClientService::class)
                    ->getAllForUser()
            )->getItemsFromModelSelected([$this->accountSearchFilter->getClientId()])
        );
        $this->view->assign(
            'categories',
            SelectItemAdapter::factory(
        ...
        $this->view->assign(
            'stickySearchBar',
            $this->context->getUserData()->getPreferences()->isStickySearchBar()
        );
    }
    ...
    public function getAccountSearch(): void
    {
        ...
        if (AccountSearchItem::$wikiEnabled) {
            $wikiFilter = array_map(fn($value): string => preg_quote((string) $value, '/'), $this->configData->getWikiFilter());
     ...
     * @throws NotFoundException
     */
    private function getGrid(): \SP\Html\DataGrid\DataGrid
    {
     ...
     */
    private function getHeaderSort(): \SP\Html\DataGrid\Layout\DataGridHeaderSort
    {
        $icons = $this->view->getTheme()->getIcons();
