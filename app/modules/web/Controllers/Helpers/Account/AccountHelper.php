...
final class AccountHelper extends HelperBase
{
     ...
     */
    public function setViewForAccount(AccountDetailsResponse $accountDetailsResponse, $actionId): void
    {
        ...
        $usersView = SelectItemAdapter::getIdFromArrayOfObjects(
            array_filter($accountDetailsResponse->getUsers(), fn(\SP\DataModel\ItemData $value): bool => (int) $value->isEdit === 0)
        );

        $usersEdit = SelectItemAdapter::getIdFromArrayOfObjects(
            array_filter($accountDetailsResponse->getUsers(), fn(\SP\DataModel\ItemData $value): bool => (int) $value->isEdit === 1)
        );

        $userGroupsView = SelectItemAdapter::getIdFromArrayOfObjects(
            array_filter($accountDetailsResponse->getUserGroups(), fn(\SP\DataModel\ItemData $value): bool => (int) $value->isEdit === 0)
        );

        $userGroupsEdit = SelectItemAdapter::getIdFromArrayOfObjects(
            array_filter($accountDetailsResponse->getUserGroups(), fn(\SP\DataModel\ItemData $value): bool => (int) $value->isEdit === 1)
        );
        ...
        $this->view->assign('isModified', $accountData->getDateEdit() !== null && strtotime($accountData->getDateEdit()) !== false);
     ...
     * @throws ServiceException
     */
    public function setViewForBlank($actionId): void
    {
        $this->actionId = $actionId;
     ...
     * @throws ServiceException
     */
    public function setViewForRequest(AccountDetailsResponse $accountDetailsResponse, $actionId): bool
    {
        $this->accountId = $accountDetailsResponse->getAccountVData()->getId();
        $this->actionId = $actionId;
        $this->accountAcl = new AccountAcl($actionId);
