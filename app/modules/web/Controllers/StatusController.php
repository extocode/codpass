    ...
    public function checkReleaseAction()
    {
        try {
            ...
            if ($request->getStatusCode() === 200
                ...
                $requestData = json_decode((string) $request->getBody());
                ...
                if ($requestData !== null && !isset($requestData->message) && preg_match('/v?(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)\.(?P<build>\d+)(?P<pre_release>\-[a-z0-9\.]+)?$/', (string) $requestData->tag_name, $matches)) {
