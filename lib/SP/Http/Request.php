...
final class Request
{
    ...
    private readonly \Klein\Request $request;
    ...
    public static function getSecureAppPath(string $path, ?string $base = null): string
    {
        ...
        if ($realPath === false
            || !str_starts_with($realPath, (string) $base)
        ) {
            return '';
        }
    ...
    public function getForwardedFor(): ?array
    {
        ...
        if ($forwarded !== null &&
            ...
            return array_filter(array_merge($matches[1], $matches[2]), fn(string $value): bool => $value !== '' && $value !== '0');
        }
    ...
    public function analyzeArray(string $param, ?callable $mapper = null, $default = null)
    {
        ...
        if ($requestValue !== null
            ...
            return array_map(fn($value): int|string => is_numeric($value) ? Filter::getInt($value) : Filter::getString($value), $requestValue);
        }
    ...
    {
        return str_contains((string) $this->headers->get('Accept'), 'application/json');
    }
    ...
    public function getForwardedData(): ?array
    {
        ...
        if ($forwarded !== '' && $forwarded !== '0'
            ...
            if (isset($data['proto']) && ($data['proto'] !== '' && $data['proto'] !== '0') && !empty($data['host'])) {
                return $data;
            }
        }
    ...
    public function getXForwardedData(): ?array
    {
        ...
        // Check (deprecated) de facto standard
        if ($forwardedHost !== '' && $forwardedHost !== '0' && ($forwardedProto !== '' && $forwardedProto !== '0')) {
            $data = [
            ...
            // Check if protocol and host are not empty
            if (isset($data['host']) && ($data['host'] !== '' && $data['host'] !== '0') && (isset($data['proto']) && ($data['proto'] !== '' && $data['proto'] !== '0'))) {
                return $data;
            }
        }
