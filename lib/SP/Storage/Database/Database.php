...
final class Database implements DatabaseInterface
{
     ...
     */
    private string|bool|null $lastId = null;
     ...
     * @throws QueryException
     */
    private function prepareQueryData(QueryData $queryData, bool|array $isCount = false, array $options = [])
    {
        ...
        try {
            ...
            if ((int) $e->getCode() === 23000) {
                $message = __u('Integrity constraint');
                $hint = $e->getMessage();

                // Check for foreign key constraint violation on delete (error 1451)
                if (str_contains($hint, '1451')) {
                    $message = __u('Cannot delete: this item is being used by other records');

                    // Try to extract the table name that has the foreign key reference
                    // Pattern: foreign key constraint fails (`database`.`TableName`, CONSTRAINT ...
                    if (preg_match('/constraint fails \(`[^`]+`\.`([^`]+)`/i', $hint, $matches)) {
                        $dependentTable = $matches[1];
                        $hint = sprintf(
                            __('This item cannot be deleted because it is referenced by records in "%s". Please remove or reassign those records first.'),
                            $dependentTable
                        );
                    }
                }
