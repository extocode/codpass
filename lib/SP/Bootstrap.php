...
final class Bootstrap
{
    ...
    protected function initRouter(): void
    {
        ...
        $this->router->onError(function ($router, string $err_msg, $type, $err): void {
        ...
        $this->router->respond(
            ['POST'],
            '@/api\.php',
            function ($request, \Klein\Response $response, $service) use ($oops) {
                try {
                    logger('API route');

                    $apiRequest = self::$container->get(ApiRequest::class);

                    [$controller, $action] = explode('/', (string) $apiRequest->getMethod());
        ...
        $this->router->respond(
            ...
            function ($request, $response, $service) use ($oops) {
                try {
                    ...
                    $methodName = empty($matches['action'][0]) ? 'indexAction' : $matches['action'][0] . 'Action';
    ...
    public function initPHPVars(): void
    {
        if (defined('DEBUG') && DEBUG) {
            Debug::enable();
        } elseif (($this->router->request()->cookies()->get('XDEBUG_SESSION')
     ...
     * @throws NotFoundException
     */
    public static function run(Container $container, $module = APP_MODULE): void
    {
        $bs = new self($container);
