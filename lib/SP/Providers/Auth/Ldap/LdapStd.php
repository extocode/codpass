...
{
    public const FILTER_USER_OBJECT = '(|(objectClass=inetOrgPerson)(objectClass=person)(objectClass=simpleSecurityObject))';

    public const FILTER_GROUP_OBJECT = '(|(objectClass=groupOfNames)(objectClass=groupOfUniqueNames)(objectClass=group))';

    public const FILTER_USER_ATTRIBUTES = ['samaccountname', 'cn', 'uid', 'userPrincipalName'];

    public const FILTER_GROUP_ATTRIBUTES = ['memberOf', 'groupMembership'];
    ...
    private function checkUserInGroupByFilter(string $userDn): bool
    {
        ...
        $userDN = ldap_escape($userDn, '', LDAP_ESCAPE_FILTER);
        $filter = '(&(cn=' . $groupName . ')'
