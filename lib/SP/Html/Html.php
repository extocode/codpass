...
final class Html
{
    ...
    public static function sanitize(&$data): ?string
    {
        ...
        if (is_array($data)) {
            ...
            $data = html_entity_decode((string) $data, ENT_COMPAT, 'UTF-8');

            // Remove any attribute starting with "on" or xmlns
            $data = preg_replace(/** @lang RegExp */
                '#(<[^>]+?[\x00-\x20\x2f"\'])(?:on|xmlns)[^>]*+>#iu',
                '$1>',
                $data
            );
     ...
     */
    public static function truncate($text, $limit, string $ellipsis = '...')
    {
     ...
     */
    public static function anchorText($text, $link = null, $title = null, $attribs = ''): string
    {
        $alink = $link ?? $text;
        $atitle = $title ?? $text;
    ...
    public static function getSafeUrl(string $url): string
    {
        ...
        return preg_replace_callback(
            '/["<>\']+/u',
            fn($matches): string => urlencode((string) $matches[0]),
            strip_tags($url)
        );
    }
}
