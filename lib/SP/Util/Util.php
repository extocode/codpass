...
final class Util
{
    ...
    public static function getTempDir()
    {
        ...
        $checkDir = function (string $dir) use ($file): string|false {
            if (file_exists($dir . DIRECTORY_SEPARATOR . $file)) {
                return $dir;
            }

            if ((is_dir($dir) || @mkdir($dir)) && touch($dir . DIRECTORY_SEPARATOR . $file)) {
                return $dir;
            }
            ...
            if ($process) {
                ...
                if ($srcClass !== null) {
                    $serialized = preg_replace_callback(
                        ...
                        fn($matches): string => ':' . strlen((string) $matches[1]) . ':"' . $matches[1] . '"',
                        $serialized
                    );
                }
     ...
     * @param $class
     */
    public static function castToClass($cast, string $class): mixed
    {
        // TODO: should avoid '__PHP_Incomplete_Class'?

        $cast = is_object($cast) ? serialize($cast) : $cast;

        return unserialize(preg_replace('/O:\d+:"[^"]++"/', 'O:' . strlen($class) . ':"' . $class . '"', (string) $cast));
    }
     ...
     */
    public static function lockApp($userId, $subject): void
    {
    ...
    {
        return array_map(intval(...), explode($delimiter, $itemsId));
    }
