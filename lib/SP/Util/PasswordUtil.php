...
{
    public const CHARS = 'abcdefghijklmnopqrstuwxyz';

    public const CHARS_SPECIAL = '@$%&/()!_:.;{}^-';

    public const CHARS_NUMBER = '0123456789';
     ...
     */
    public static function randomPassword($length = 16, ?int $flags = null): string
    {
        if ($flags === null) {
         ...
         * @return array
         */
        $passGen = function () use ($alphabet, $length): array {
            $pass = [];
        ...
        if (($flags & self::FLAG_PASSWORD_STRENGTH) !== 0) {
            do {
                $pass = $passGen();
    ...
    public static function checkStrength(array $pass): array
    {
        ...
        foreach ($pass as $char) {
            $strength['lower'] += substr_count(self::CHARS, (string) $char);
            $strength['upper'] += substr_count($charsUpper, (string) $char);
            $strength['special'] += substr_count(self::CHARS_SPECIAL, (string) $char);
            $strength['number'] += substr_count(self::CHARS_NUMBER, (string) $char);
        }
