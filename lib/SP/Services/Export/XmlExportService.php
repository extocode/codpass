...
final class XmlExportService extends Service
{
    ...
    private \DOMNode|bool|null $root = null;
    ...
    private function deleteOldExports(): void
    {
        ...
        array_map(fn($file): bool => @unlink($file), array_merge(glob($path . '_export-*'), glob($path . '*.xml')));
    }
    ...
    private function createMeta(): void
    {
        try {
            ...
            $metaTime = $this->xml->createElement('Time', (string) time());
            $metaUser = $this->xml->createElement('User', $userData->getLogin());
            $metaUser->setAttribute('id', (string) $userData->getId());
            $metaGroup = $this->xml->createElement('Group', $userData->getUserGroupName());
            $metaGroup->setAttribute('id', (string) $userData->getUserGroupId());
     ...
     */
    private function escapeChars($data): string|array
    {
        if ($data === null) {
            return '';
        }
    ...
    private function createAccounts(): void
    {
        try {
            ...
            foreach ($accounts as $account) {
                ...
                $accountCustomerId = $this->xml->createElement('clientId', (string) $account->getClientId());
                $accountCategoryId = $this->xml->createElement('categoryId', (string) $account->getCategoryId());
