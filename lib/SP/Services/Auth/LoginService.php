...
final class LoginService extends Service
{
     ...
     * Estados
     */
    public const STATUS_INVALID_LOGIN = 1;

    public const STATUS_INVALID_MASTER_PASS = 2;

    public const STATUS_USER_DISABLED = 3;

    public const STATUS_NEED_OLD_PASS = 5;

    public const STATUS_MAX_ATTEMPTS_EXCEEDED = 6;

    public const STATUS_PASS_RESET = 7;

    public const STATUS_PASS = 0;

    public const STATUS_NONE = 100;

    private ?\SP\DataModel\UserLoginData $userLoginData = null;
    ...
    {
        $__t0 = microtime(true);
        $__lap = static function (string $phase) use (&$__t0): void {
            $now = microtime(true);
            logger(sprintf('LOGIN-TIMING %s: %.1f ms', $phase, ($now - $__t0) * 1000), 'INFO');
            $__t0 = $now;
        };
        ...
        $this->loadMasterPass();
        $__lap('loadMasterPass');
        $this->setUserSession();
        $__lap('setUserSession');
        $this->loadUserPreferences();
        $__lap('loadUserPreferences');
        $this->cleanUserData();
     ...
     * @throws AuthException
     */
    private function checkUser(): \SP\Services\Auth\LoginResponse
    {
    ...
    protected function initialize(): void
    {
        ...
        $this->trackRequest = $this->trackService->getTrackRequest(self::class);
    }
