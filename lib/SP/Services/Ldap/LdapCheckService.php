...
final class LdapCheckService extends Service
{
     ...
     * @throws LdapException
     */
    public function getObjects($includeGroups = true): array
    {
        ...
        $data['results'][] = [
            ...
            'items' => $this->ldapResultsMapper(
                $ldapActions->getObjects(
                    $this->ldap->getGroupMembershipFilter(),
                    ['dn']
                )
            ),
        ];
        ...
        if ($includeGroups) {
            $data['results'][] = [
                ...
                'items' => $this->ldapResultsMapper(
                    $ldapActions->getObjects(
                        $this->ldap->getGroupObjectFilter(),
                        ['dn']
                    )
                ),
            ];
        ...
        array_walk($data['results'], function (array $value) use (&$data): void {
     ...
     */
    public function ldapResultsMapper($data, $attribute = 'dn'): array
    {
        $out = [];
        ...
        foreach ($data as $result) {
            if (is_array($result)) {
                foreach ($result as $ldapAttribute => $value) {
                    if (strtolower((string) $ldapAttribute) === $attribute) {
                        $out[] = $value;
                    }
                }
            }
     ...
     * @throws LdapException
     */
    public function getObjectsByFilter($filter): array
    {
