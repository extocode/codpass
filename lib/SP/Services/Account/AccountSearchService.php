...
final class AccountSearchService extends Service
{
    ...
    public const COLORS_CACHE_FILE = CACHE_PATH . DIRECTORY_SEPARATOR . 'colors.cache';
    ...
    private ?\SP\Storage\File\FileCache $colorCache = null;
     ...
     */
    public function analyzeQueryFilters($string): \SP\Mvc\Model\QueryCondition
    {
        ...
        if ($match !== false && $match > 0) {
            if (isset($filters['search'][0]) && ($filters['search'][0] !== '' && $filters['search'][0] !== '0')) {
            ...
            if ($filtersAndValues !== []) {
                $filtersItem = array_filter($filtersAndValues, fn($value, $key): bool => in_array($key, self::FILTERS['items']['subject'], true)
                ...
                $filtersOperator = array_filter($filtersAndValues, fn($value, $key): bool => in_array($key, self::FILTERS['operator']['subject'], true)
                    && in_array($value, self::FILTERS['operator']['condition'], true), ARRAY_FILTER_USE_BOTH);

                if ($filtersOperator !== []) {
                    $this->processFilterOperator($filtersOperator);
                }

                $filtersCondition = array_filter(array_map(function (string $subject, string $condition): ?string {
    ...
    private function processFilterItems(array $filters, QueryCondition $queryCondition): void
    {
    ...
    {
        $this->filterOperator = match ($filters['op']) {
            'and' => QueryCondition::CONDITION_AND,
            'or' => QueryCondition::CONDITION_OR,
            default => $this->filterOperator,
        };
    }

    private function processFilterIs(array $filters, QueryCondition $queryCondition): void
    {
    ...
    protected function getCacheForAccount(AccountSearchVData $accountSearchData)
    {
        ...
        $dateEdit = $accountSearchData->getDateEdit();
        if ($cache === false
            || !isset($cache[$accountId])
            || ($dateEdit !== null && $cache[$accountId]->getTime() < (int) strtotime($dateEdit))
        ) {
