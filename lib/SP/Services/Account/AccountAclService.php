...
final class AccountAclService extends Service
{
    ...
    private ?\SP\DataModel\Dto\AccountAclDto $accountAclDto = null;

    private ?\SP\Services\Account\AccountAcl $accountAcl = null;
     ...
     * @param $userId
     */
    public static function clearAcl(string $userId): bool
    {
    ...
    public function getAcl($actionId, ?AccountAclDto $accountAclDto = null, $isHistory = false)
    {
        ...
        if ($accountAclDto instanceof \SP\DataModel\Dto\AccountAclDto) {
     ...
     * @return AccountAcl
     */
    public function getAclFromCache($accountId, $actionId): ?\SP\Services\Account\AccountAcl
    {
        try {
     ...
     * @param int $actionId
     */
    public function getCacheFileForAcl($accountId, $actionId): string
    {
    ...
    private function compileAccountAccess(): void
    {
        ...
        $userGroups = array_map(fn($value): int => (int) $value->userGroupId, $userToUserGroupService->getGroupsForUser($this->userData->getId()));
     ...
     * @param $userId
     */
    private function getUserInSecondaryUsers($userId): array
    {
        return array_values(array_filter($this->accountAclDto->getUsersId(), fn(\SP\DataModel\ItemData $value): bool => (int) $value->id === $userId));
    }
     ...
     * @return bool|array
     */
    private function getUserGroupsInSecondaryGroups(array $userGroups, $userGroupId): array
    {
        ...
        return array_values(array_filter(
            ...
            fn(\SP\DataModel\ItemData $value): bool => (int) $value->id === $userGroupId
