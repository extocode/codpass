...
final class TemporaryMasterPassService extends Service
{
     ...
     */
    public const PARAM_PASS = 'tempmaster_pass';

    public const PARAM_KEY = 'tempmaster_passkey';

    public const PARAM_HASH = 'tempmaster_passhash';
    ...
    public function checkTempMasterPass($pass)
    {
        try {
            ...
            if (($passTime !== 0 && time() > $passMaxTime)
     ...
     */
    public function sendByEmailForGroup($groupId, $key): void
    {
        $mailMessage = $this->getMessageForEmail($key);

        $emails = array_map(fn($value) => $value->email, $this->dic->get(UserService::class)->getUserEmailForGroup($groupId));
     ...
     * @throws QueryException
     */
    public function sendByEmailForAllUsers($key): void
    {
        $mailMessage = $this->getMessageForEmail($key);

        $emails = array_map(fn($value) => $value->email, $this->dic->get(UserService::class)->getUserEmailForAll());
     ...
     * @param $key
     */
    private function getMessageForEmail($key): \SP\Core\Messages\MailMessage
    {
