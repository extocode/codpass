...
final class FileBackupService extends Service
{
     ...
     * @throws ServiceException
     */
    private function checkBackupDir(): bool
    {
    ...
    public static function getDbBackupFilename(string $path, string $hash, bool $compressed = false): string
    {
    ...
    private function deleteOldBackups(): void
    {
        ...
        array_map(fn($file): bool => @unlink($file), array_merge(
            glob($path . '_db-*'),
            glob($path . '_app-*'),
            glob($path . '*.sql')
        ));
     ...
     */
    private function backupTables(FileHandler $fileHandler, string $tables = '*'): void
    {
        $this->eventDispatcher->notifyEvent(
            'run.backup.process',
            new Event(
        ...
        foreach ($resTables as $table) {
            ...
            if (isset($txtCreate->{'Create Table'})) {
                $sqlOut = '-- ' . PHP_EOL;
                $sqlOut .= '-- Table ' . strtoupper((string) $tableName) . PHP_EOL;
                $sqlOut .= '-- ' . PHP_EOL;
                ...
                $sqlOutViews .= '-- ' . PHP_EOL;
                $sqlOutViews .= '-- View ' . strtoupper((string) $tableName) . PHP_EOL;
                $sqlOutViews .= '-- ' . PHP_EOL;
        ...
        foreach ($resTables as $tableName) {
            ...
            if (strrpos((string) $tableName, '_v') !== false) {
                continue;
            }
