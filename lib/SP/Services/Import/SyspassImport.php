...
final class SyspassImport extends XmlImportBase implements ImportInterface
{
    ...
    protected function processEncrypted(): void
    {
        ...
        foreach ($this->xmlDOM->getElementsByTagName('Data') as $node) {
            /** @var $node DOMElement */
            $data = base64_decode((string) $node->nodeValue);
    ...
    protected function processCategories(): void
    {
        $this->getNodesData(
            'Categories',
            'Category',
            function (DOMElement $category): void {
                $categoryData = new CategoryData();

                foreach ($category->childNodes as $node) {
                    if (property_exists($node, 'tagName') && $node->tagName !== null) {
                        switch ($node->tagName) {
                            case 'name':
    ...
    protected function processClients(): void
    {
        $this->getNodesData(
            'Clients',
            'Client',
            function (DOMElement $client): void {
    ...
    protected function processTags(): void
    {
        $this->getNodesData(
            'Tags',
            'Tag',
            function (DOMElement $tag): void {
                $tagData = new TagData();

                foreach ($tag->childNodes as $node) {
                    if (property_exists($node, 'tagName') && $node->tagName !== null && $node->tagName === 'name') {
    ...
    {
        // Debug: Check if Accounts node exists and has data
        $accountsNode = $this->xmlDOM->getElementsByTagName('Accounts');
        $accountNodes = $this->xmlDOM->getElementsByTagName('Account');

        $debugInfo = sprintf(
            'DEBUG: Accounts=%d, Account=%d, Categories=%d, Clients=%d',
            $accountsNode->length,
            $accountNodes->length,
            count($this->items['category'] ?? []),
            count($this->items['client'] ?? [])
        );

        // Throw if no accounts found to make the error visible
        if ($accountNodes->length === 0) {
            throw new ImportException('No Account nodes found in XML. ' . $debugInfo);
        }

        $this->eventDispatcher->notifyEvent(
            'run.import.syspass.debug',
            new Event($this, EventMessage::factory()
                ->addDescription($debugInfo))
        );

        $this->getNodesData(
            'Accounts',
            'Account',
            function (DOMElement $account): void {
                $accountRequest = new AccountRequest();

                // Debug: track raw values
                $debugCategoryId = null;
                $debugClientId = null;
                    ...
                    if (isset($node->tagName)) {
                        switch ($node->tagName) {
                            ...
                            case 'login':
                                ...
                                break;
                            case 'categoryId':
                                $debugCategoryId = (int) $node->nodeValue;
                                $accountRequest->categoryId = $this->getWorkingItem('category', $debugCategoryId);
                                break;
                            case 'clientId':
                            case 'customerId':
                                $debugClientId = (int) $node->nodeValue;
                                $accountRequest->clientId = $this->getWorkingItem('client', $debugClientId);
                                break;
                            case 'url':
                ...
                // Debug: Log account details before import attempt
                $this->eventDispatcher->notifyEvent(
                    'run.import.syspass.debug.account',
                    new Event($this, EventMessage::factory()
                        ->addDescription('DEBUG: Attempting to import account')
                        ->addDetail('Name', $accountRequest->name ?? 'NULL')
                        ->addDetail('XML categoryId', (string) ($debugCategoryId ?? 'NULL'))
                        ->addDetail('Resolved categoryId', (string) ($accountRequest->categoryId ?? 'NULL'))
                        ->addDetail('XML clientId', (string) ($debugClientId ?? 'NULL'))
                        ->addDetail('Resolved clientId', (string) ($accountRequest->clientId ?? 'NULL')))
                );
                ...
                try {
                    ...
                    // DEBUG: Throw the first error to see what's wrong
                    throw new ImportException(sprintf(
                        'IMPORT FAILED for "%s": %s | catId=%s, clientId=%s',
                        $accountRequest->name ?? 'unknown',
                        $e->getMessage(),
                        $accountRequest->categoryId ?? 'NULL',
                        $accountRequest->clientId ?? 'NULL'
                    ));
                }
            }
        );
