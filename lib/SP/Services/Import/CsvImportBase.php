...
abstract class CsvImportBase
{
     ...
     * @param int $numFields
     */
    public function setNumFields($numFields): void
    {
     ...
     */
    public function setMapFields($mapFields): void
    {
    ...
    protected function processAccounts()
    {
        ...
        while (($fields = fgetcsv($handler, 0, $this->importParams->getCsvDelimiter())) !== false) {
            ...
            [$accountName, $clientName, $categoryName, $url, $login, $password, $notes] = $fields;

            try {
                if (in_array($clientName, [null, '', '0'], true) || in_array($categoryName, [null, '', '0'], true)) {
