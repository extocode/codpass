...
final class Installer extends Service
{
    ...
    public const VERSION_TEXT = '4.0.2';

    public const BUILD = 26062603;
    ...
    private ?\SP\Services\Install\InstallData $installData = null;
     ...
     */
    public function run(InstallData $installData): self
    {
    ...
    private function setupDbHost(): void
    {
        if (preg_match('/^(?:(?P<host>.*):(?P<port>\d{1,5}))|^(?:unix:(?P<socket>.*))/', $this->installData->getDbHost(), $match)) {
            if (isset($match['socket']) && ($match['socket'] !== '' && $match['socket'] !== '0')) {
        ...
        if (!str_contains('localhost', $this->installData->getDbHost())
            && !str_contains('127.0.0.1', $this->installData->getDbHost())) {
            $address = APP_MODULE === 'tests' ? SELF_IP_ADDRESS : $this->request->getServer('SERVER_ADDR');
     ...
     * @throws SPException
     */
    private function setupDb($type = 'mysql'): void
    {
        if ($type === 'mysql') {
