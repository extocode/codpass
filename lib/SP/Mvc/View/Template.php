...
final class Template
{
     ...
     */
    private function checkTemplate(string $template, $base = null): string
    {
        $base ??= $this->base ?? null;

        if ($base === null) {
            $templateFile = $this->theme->getViewsPath() . DIRECTORY_SEPARATOR . $template . self::TEMPLATE_EXTENSION;
        } elseif (str_starts_with($base, (string) APP_ROOT)
            && is_dir($base)
        ) {
     ...
     */
    private function setContentTemplate($file, $name): void
    {
     ...
     * @param $name
     */
    public function removeTemplate($name): self
    {
     ...
     * @param $name
     */
    public function removeContentTemplate($name): self
    {
     ...
     * @param $partial
     */
    public function addPartial($partial): void
    {
     ...
     * @param string $name Nombre de la plantilla
     */
    private function setTemplate($file, $name): void
    {
    ...
    public function render(): string|false
    {
        ...
        $_getRoute = function ($path) use ($sk, $configData): string {
     ...
     */
    public function append($name, $value, $scope = null, $index = null): void
    {
        if (null !== $scope) {
            $name = $scope . '_' . $name;
        }
     ...
     * Reset de las plantillas añadidas
     */
    public function resetContentTemplates(): self
    {
    ...
    public function hashContentTemplates(): bool
    {
     ...
     * @param null $scope string ámbito de la variable
     */
    public function assign($name, $value = '', $scope = null): void
    {
        if (null !== $scope) {
            $name = $scope . '_' . $name;
        }
