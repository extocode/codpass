...
final class AccountHistoryRepository extends Repository implements RepositoryItemInterface
{
     ...
     */
    public function update($itemData): never
    {
    ...
    /**
     * Deletes all the items for given client ids
     *
     * @throws ConstraintException
     * @throws QueryException
     */
    public function deleteByClientIdBatch(array $ids): int
    {
        if ($ids === []) {
            return 0;
        }
        ...
        $queryData = new QueryData();
        $queryData->setQuery('DELETE FROM AccountHistory WHERE clientId IN (' . $this->getParamsFromArray($ids) . ')');
        $queryData->setParams($ids);
        $queryData->setOnErrorMessage(__u('Error while deleting the account history'));
