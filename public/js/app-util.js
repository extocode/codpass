        ...
        const handleFiles = function (filesArray) {
            ...
            for (let i = 0; i < filesArray.length; i++) {
                ...
                if (checkFileSize(file.size)) {
                    sysPassApp.msg.error(
                        sysPassApp.config.LANG[18] +
                            "<br>" +
                            file.name +
                            " (Max: " +
                            sysPassApp.config.FILES.MAX_SIZE +
                            ")",
                    );
                } else if (!checkFileMimeType(file.type)) {
                    sysPassApp.msg.error(
                        sysPassApp.config.LANG[19] + "<br>" + file.type,
                    );
                } else {
    ...
    const password = {
        ...
        random: function (callback) {
            ...
            const checkComplexity = function (inPass) {
                ...
                if (password.config.complexity.symbols) {
                    const res = inPassArray.some(function (el) {
                ...
                if (
                    password.config.complexity.chars &&
        ...
        output: function (level, $target) {
            ...
            if (this.config.passLength === 0) {
                ...
                $passLevel
                    .attr("data-level-msg", sysPassApp.config.LANG[11])
                    .addClass("weak");
            } else if (score === 0) {
                $passLevel
                    .attr(
                        "data-level-msg",
                        sysPassApp.config.LANG[9] +
                            " - " +
                ...
                $passLevel
                    .attr("data-level-msg", sysPassApp.config.LANG[7])
                    .addClass("strong");
            } else if (score === 4) {
                $passLevel
                    .attr("data-level-msg", sysPassApp.config.LANG[10])
                    .addClass("strongest");
            }
        },
