...
sysPass.Main = function () {
            ...
            if (typeof oPublic.plugins[plugin].checkVersion === "function") {
                oPublic.plugins[plugin].checkVersion().then(function (json) {
                    if (json.status === 0 && json.data.plugin !== undefined) {
                        msg.info(
                            String.format(
                                oPublic.config.LANG[67],
                                json.data.plugin,
                                json.data.remoteVersion,
                            ),
                        );
                    }
                });
    ...
    const initializeClipboard = function () {
        ...
        $("body")
            .on("click", ".clip-pass-button", function () {
                const json = oPublic.actions.account
                    .copyPass($(this))
                    .done(function (json) {
                        if (json.status !== 0) {
                            msg.out(json);
            ...
            .on("click", ".dialog-clip-button", function () {
                ...
                clipboard.copy($target.text().replace(/[\r\n]+/g, "")).then(
                    function () {
            ...
            .on("click", ".clip-pass-icon", function () {
                ...
                clipboard.copy(oPublic.util.decodeEntities($target.val())).then(
                    function () {
                        msg.ok(oPublic.config.LANG[45]);
                    },
                    function (err) {
    ...
    const encryptFormValue = function ($input) {
        ...
        // ENCRYPTION DISABLED - RSA incompatibility with phpseclib3
        // if (curValue !== ""
    ...
    const checkLogout = function () {
        ...
        if (getUrlVars("r") === "login/logout") {
            // Logout is intentional; skip the "Session finished" warning toast and
        ...
        out: function (data) {
            if (typeof data === "object") {
                ...
                if (data.messages !== undefined && data.messages.length > 0) {
                    description =
                        description + "<br>" + data.messages.join("<br>");
                }

                // Check if error response includes an action option (e.g., clear history and retry)
                if (status === 1 && data.data && data.data.action) {
                    const actionData = data.data;
                    const self = this;

                    mdlDialog().show({
                        text: '<div id="alert"><p id="alert-text">' + description + '</p></div>',
                        negative: {
                            title: oPublic.config.LANG[44],
                            onClick: function (e) {
                                e.preventDefault();
                            },
                        },
                        positive: {
                            title: actionData.actionDescription || oPublic.config.LANG[43],
                            onClick: function (e) {
                                e.preventDefault();

                                const opts = oPublic.requests.getRequestOpts();
                                opts.url = oPublic.actions.ajaxUrl.entrypoint + "?r=" + actionData.action;
                                opts.data = {
                                    sk: oPublic.sk.get(),
                                    items: actionData.ids || []
                                };

                                oPublic.requests.getActionCall(opts, function (json) {
                                    self.out(json);

                                    // Refresh the current view if successful
                                    if (json.status === 0) {
                                        const $currentTab = $("#content").find("[id^='tabs-'].is-active");
                                        if ($currentTab.length > 0) {
                                            oPublic.actions.getContent({
                                                r: $currentTab.data("tab-route"),
                                                tabIndex: $currentTab.data("tab-index")
                                            });
                                        }
                                    }
                                });
    ...
    const getEnvironment = function () {
        ...
        const root =
            window.location.protocol + "//" + window.location.host + rootPath();
        ...
        return oPublic.requests
            .getActionCall(opts, function (json) {
                if (json.data !== undefined) {
                    ...
                    configHandler.setCheckNotifications(
                        json.data.check_notifications,
                    );
                    ...
                    configHandler.setFileAccountAllowedMime(
                        json.data.files_allowed_mime,
                    );
                    configHandler.setFileImportAllowedMime(
                        json.data.import_allowed_mime,
                    );
                    ...
                    configHandler.setAuthBasicAutologinEnabled(
                        json.data.authbasic_autologin,
                    );
            ...
            .fail(function () {
                msg.error(
                    "Error while getting sysPass config<br/>Please try again or check web server logs",
                );
            });
    };
        ...
        getEnvironment().then(function () {
            if (!checkLogout()) {
                ...
                if (oPublic.config.PLUGINS.length > 0) {
                    ...
                    if (
                        oPublic.config.AUTH.LOGGEDIN === true &&
    ...
    const getUrlVars = function (param) {
        ...
        return param !== undefined && vars[param] !== undefined
            ? vars[param]
            : vars;
    };
