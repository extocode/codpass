...
sysPass.Actions = function (log) {
    ...
    const updateActiveNav = function (route) {
        log.info("updateActiveNav: " + route);

        // Remove active class from all nav items and user menu
        $(".mdl-navigation__link").removeClass("nav-active");
        $("#users-menu-lower-right").removeClass("nav-active");

        // Only proceed if route is a valid string
        if (typeof route !== "string" || !route) {
            return;
        }

        // Find and activate matching nav item by exact route only
        $(".mdl-navigation__link[data-route='" + route + "']").addClass("nav-active");

        // Mark user menu as active for user settings routes
        if (route.startsWith("userSettings")) {
            $("#users-menu-lower-right").addClass("nav-active");
        }
    };
        ...
        $.magnificPopup.open({
            ...
            callbacks: {
                ...
                close: function () {
                    if (
                        callback !== undefined &&
                        typeof callback.close === "function"
                    ) {
                        callback.close();
                    }
                },
    ...
    const showImageBox = function ($obj, response) {
        const $content = $(
            '<div id="box-popup" class="image">' + response + "</div>",
        );
    ...
    const account = {
        view: function ($obj) {
            ...
            var actionTab = $obj.data("action-tab");
            getContent(
                ...
                if (actionTab) {
                    setTimeout(function () {
                        var $tab = $(
                            "a.mdl-tabs__tab[href='#" + actionTab + "']",
                        );
                        if ($tab.length) {
                            $tab[0].click();
                        }
        ...
        viewHistory: function ($obj) {
            ...
            getContent(
                { r: [$obj.data("action-route"), $obj.val()] },
                "account",
            );
        },
        edit: function ($obj) {
        ...
        saveFavorite: function ($obj, callback) {
            ...
            const actionRoute = isOn
                ? $obj.data("action-route-off")
                : $obj.data("action-route-on");
        ...
        sort: function ($obj) {
            ...
            const dir = $obj.data("dir");

            if (dir === -1) {
                // Clear sort filter
                $frmSearch.find('input[name="skey"]').val("");
                $frmSearch.find('input[name="sorder"]').val("");
            } else {
        ...
        editPass: function ($obj) {
            ...
            const parentId = $obj.data("parent-id");
            const itemId =
                parentId === undefined ? $obj.data("item-id") : parentId;
        ...
        saveEditRestore: function ($obj) {
            ...
            sysPassApp.requests.getActionCall(opts, function (json) {
                ...
                if (
                    json.data.itemId !== undefined &&
                    json.data.nextAction !== undefined
                ) {
                    getContent(
                        { r: [json.data.nextAction, json.data.itemId] },
                        "account",
                    );
                }
            });
        ...
        save: function ($obj) {
            ...
            $("select.select-box-tags[data-hash][data-updated=true]").each(
            ...
            sysPassApp.requests.getActionCall(opts, function (json) {
                ...
                if (
                    ...
                    // Remove edit page from history so back button goes to previous page
                    sysPassApp.requests.history.del();
            ...
            sysPassApp.requests.getActionCall(opts, function (json) {
                ...
                switch (json.status) {
                    ...
                    case 5:
                        ...
                        $("#soldpass").addClass("extra-visible");
                        break;
                    default:
                        sysPassApp.msg.out(json);
            ...
            mdlDialog().show({
                ...
                positive: {
                    ...
                    onClick: function (e) {
                        ...
                        opts.data =
                            $obj.serialize() + "&sk=" + sysPassApp.sk.get();
    ...
    const file = {
        ...
        download: function ($obj) {
            ...
            const url = sysPassApp.requests.getUrl(
                sysPassApp.util.getUrl(ajaxUrl.entrypoint, {
                    r: [$obj.data("action-route"), $obj.data("item-id")],
                    sk: sysPassApp.sk.get(),
                }),
            );
        ...
        save: function ($obj) {
            ...
            const request = function (notify) {
                ...
                sysPassApp.requests.getActionCall(opts, function (json) {
                    ...
                    if (json.status === 0) {
                        getContent({
                            r: [$obj.data("action-next"), accountId],
                        });
                    }
                });
            };
    ...
    const appMgmt = {
        show: function ($obj) {
            ...
            opts.url = sysPassApp.util.getUrl(ajaxUrl.entrypoint, {
                r: [
                    $obj.data("action-route"),
                    items.length === 0 ? $obj.data("item-id") : null,
                ],
                sk: sysPassApp.sk.get(),
                isAjax: 1,
            });
        ...
        save: function ($obj) {
            ...
            sysPassApp.requests.getActionCall(opts, function (json) {
                ...
                if (json.status === 0) {
                    ...
                    getContent({ r: $obj.data("action-next") }).then(
                        function () {
                            notification.getActive();
                        },
                    );
            ...
            sysPassApp.requests.getActionCall(opts, function (json) {
                ...
                if (
                    ...
                    showFloatingBox(json.data.template, {
                        open: function () {
                            const $list = $("#ldap-results")
                                .find(".list-wrap")
                                .empty();

                            json.data.items.forEach(function (value) {
                                $list.append(
                                    sysPassApp.theme.html.getList(
                                        value.items,
                                        value.icon,
                                    ),
                                );
                ...
                positive: {
                    ...
                    onClick: function (e) {
                        ...
                        opts.url =
                            ...
                            $obj.data("action-route");
                        opts.data =
                            $form.serialize() + "&sk=" + sysPassApp.sk.get();
    ...
    const task = {
        ...
        run: function (taskId) {
            ...
            opts.method = "get";
            opts.url = sysPassApp.util.getUrl(ajaxUrl.entrypoint, {
                r: ["task/trackStatus", taskId],
            });

            this.runner = sysPassApp.requests.getActionEvent(
                opts,
                function (result) {
